...
public class AdvertiseTruckBoardUserActivity extends AppCompatActivity implements View.OnClickListener,GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener, LocationListener {
...
//            mLatitudeText.setText(String.valueOf(mLastLocation.getLatitude()));
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.advertise_truck_board_user);
        appContext=this;
        initComponent();
    }
    ...
    private void initComponent(){
        ...
        mRecyclerView = (RecyclerView)findViewById(R.id.recyclerTruckBoard);
        ...
        ((CustomTextView)findViewById(R.id.txtAdvertiseTruck)).setOnClickListener(this);
    ...
    private void loadDataFromServer(int count_item,int skip_page,final int start_index, final int end_index,final boolean isInitial,final boolean isRefresh,final boolean isOnLoad,int request,String source,String destination){
        ...
        if(request==1){
            requestData=1;
            call = service.getTruckBoardList("created_at:-1", "" + count_item, "truck", "", "" + skip_page, jwt);
        }else{
            requestData=2;

            if(PreferenceClass.getStringPreferences(appContext, "Truck_Source_District").trim().length()>0 && PreferenceClass.getStringPreferences(appContext, "Truck_Destination_District").trim().length()>0){
                call = searchservice.getTruckBoardSearchList("available_date:-1", "" + count_item, "truck", "", "" + source, "" + destination, "" + skip_page, jwt);
            }else if(PreferenceClass.getStringPreferences(appContext, "Truck_Source_District").trim().length()>0){
                call = sourceSearchservice.getTruckBoardSourceSearchList("available_date:-1", "" + count_item, "truck", "", "" + source, "" + skip_page, jwt);
            }else if(PreferenceClass.getStringPreferences(appContext, "Truck_Destination_District").trim().length()>0){
                call = destinationSearchservice.getTruckBoardDestinationSearchList("available_date:-1", "" + count_item, "truck", "", "" + destination, "" + skip_page, jwt);
            }
        }
        ...
        call.enqueue(new Callback<TruckBoardApiResponse>() {
            ...
            public void onResponse(Response<TruckBoardApiResponse> response, Retrofit retrofit) {
                ...
                if(truckBoardPeviousList.size()==0){
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.txtAdvertiseTruck) {
            startActivity(new Intent(appContext, PostTruckActivity.class));
    ...
    private void setActionBar(){
        ...
        ((CustomTextView)mToolbar.findViewById(R.id.txtHeading)).setText(R.string.My_Truck_Board);
    }
